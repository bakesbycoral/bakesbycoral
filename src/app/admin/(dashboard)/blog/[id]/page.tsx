  ...
  const handleSubmit = async (e: React.FormEvent, submitStatus?: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (submitStatus === 'published' && originalPost?.status !== 'published') {
          router.push('/admin/blog');
        }
      }
    } catch (error) {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              ...
              {lastSaved && (
                <> &bull; Last saved {lastSaved.toLocaleTimeString()}</>
              )}
            </p>
          </div>
        </div>
      ...
      {originalPost && (
        <div className="mb-6">
          <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
            originalPost.status === 'published' ? 'bg-green-100 text-green-800' :
            originalPost.status === 'scheduled' ? 'bg-blue-100 text-blue-800' :
            'bg-gray-100 text-gray-800'
          }`}>
            {originalPost.status === 'published' ? 'Published' :
             originalPost.status === 'scheduled' ? 'Scheduled' : 'Draft'}
            {originalPost.published_at && originalPost.status === 'published' && (
              <span className="ml-2 text-xs opacity-75">
                on {new Date(originalPost.published_at).toLocaleDateString()}
              </span>
            )}
          </span>
          {originalPost.views > 0 && (
            <span className="ml-3 text-sm text-gray-500">
              {originalPost.views} view{originalPost.views !== 1 ? 's' : ''}
            </span>
          )}
        </div>
      )}
              ...
              <div className="space-y-3">
                ...
                <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500">Visible to public</div>
                  </div>
                </label>
                <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                  <input
